...
module.exports.run = async (bot, message, args) => {
    ...
    let rMemberB = message.guild.member(message.mentions.users.first());
    let amount = args[1];

    if(args[0] != rMemberB) return message.channel.send(`usage: \`${config.prefix} give @user amount\``);
